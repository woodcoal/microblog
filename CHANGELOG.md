...
- 账户、关系与通知页继续对齐 A 型 UX 基线：认证页与头部共用 1320px 内容断点，桌面导语与表单贴两端分列、移动端收为单列，并采用轻量表单阴影；认证与设置反馈补齐读屏语义，个人主页增加资料封面和所有者编辑入口；关注/粉丝改为带真实关注操作的资料行与切换导航，通知筛选明确为导航区域并避免未读状态重复提交。账户、关系和通知页面不再复用微博频道侧栏。
