...
- 账户、关系与通知页继续对齐 A 型 UX 基线：认证页与头部共用 1320px 内容断点，桌面导语与表单贴两端分列、移动端收为单列，并采用轻量表单阴影；个人主页按模板改为资料与帖子主栏、个人菜单／最近关注／推荐话题右栏，头像跨越封面与白色资料区、资料信息纵向排在头像下方，内容流只按动态／文章／讨论模块切换而不再提供综合结果；设置页改为用户中心左侧菜单与右侧编辑区；认证与设置反馈补齐读屏语义，关注/粉丝改为带真实关注操作的资料行与切换导航，通知筛选明确为导航区域并避免未读状态重复提交。账户、关系和通知页面不再复用微博频道侧栏。
