...
- 首页 Hero 右侧恢复原稿的三条竖向热门内容预览，全局置顶内容优先；移除页面下方重复的“正在被讨论”内容流。登录态“内容推荐”改为与 Hero 右侧一致的圆点栏和三条纵向预览行；两处预览行的前置图标统一为可跳转人物主页的用户头像，推荐行右侧以图标展示点赞、收藏与评论计数，并对异常缺失计数安全回退为 0。
