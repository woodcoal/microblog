(() => {
	...
	const closeAll = () =>
		document
			.querySelectorAll('.theme-menu,.nav-dropdown-menu,.admin-nav')
	...
	document.querySelectorAll('[data-theme-toggle]').forEach((button) => {
		...
		menu.innerHTML =
			themes
				.map(([id, label]) => `<button type="button" data-theme="${id}">${label}</button>`)
				.join('') +
			'<hr>' +
			accents
				.map(
					([id, label]) => `<button type="button" data-accent="${id}">● ${label}</button>`
				)
				.join('');
		...
		menu.addEventListener('click', (e) => {
			...
			if (target.dataset.theme) {
				...
				toast('已切换至 ' + target.textContent.trim());
			}
			if (target.dataset.accent) {
				root.dataset.accent = target.dataset.accent;
	...
	document.querySelectorAll('.nav-user-trigger').forEach((trigger) => {
		const menu =
			trigger.parentElement.querySelector('.nav-dropdown-menu') || trigger.nextElementSibling;
		if (!menu) return;
	...
	document
		...
		.forEach((el) => el.addEventListener('click', () => toast(el.dataset.toast)));
	...
	document.querySelectorAll('[data-like]').forEach((button) =>
		button.addEventListener('click', () => {
			...
			if (n)
				n.textContent =
					Number(n.textContent) + (button.classList.contains('liked') ? 1 : -1);
		})
	);
	...
	document.querySelectorAll('.admin-mobile-toggle').forEach((btn) =>
		btn.addEventListener('click', (e) => {
			e.stopPropagation();
			e.target.closest('.admin-shell').querySelector('.admin-nav').classList.toggle('open');
		})
	);
