...
export function parseTrendingConfig(raw?: string): TrendingConfig {
	...
	try {
		...
		if (config.version === undefined && !('wBookmarks' in config)) {
			...
			if (
				!valid(wLikes, 0, 10) ||
				!valid(wComments, 0, 10) ||
				(wLikes === 0 && wComments === 0) ||
				!valid(halfLifeHours, 1, 720)
			)
				return defaults();
			return {
				version: 'v2',
		...
		if (
			(config.version !== undefined && config.version !== 'v2') ||
			!valid(wLikes, 0, 10) ||
			!valid(wBookmarks, 0, 10) ||
			!valid(wComments, 0, 10) ||
			(wLikes === 0 && wBookmarks === 0 && wComments === 0) ||
			!valid(decayHours, 1, 720)
		)
			return defaults();
...
export function calculateTrendingScore(
	...
	const engagement =
		config.wLikes * Math.log1p(Math.max(0, signals.likes)) +
		config.wBookmarks * Math.log1p(Math.max(0, signals.bookmarks)) +
		config.wComments * Math.log1p(Math.max(0, signals.comments));
