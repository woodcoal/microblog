...
import {
	POST_PASSWORD_ACCESS_COOKIE,
	setPostPasswordAccess
} from '../src/lib/post-password-access';
...
async function setupMedia(type: 'image' | 'video') {
	...
	await writeFile(
		...
		type === 'image' ? Buffer.from('original') : Buffer.from('0123456789')
	);
	if (displayPath) {
		await mkdir(join(UPLOAD_DIR, 'protected/images/display-v1'), { recursive: true });
		await writeFile(join(UPLOAD_DIR, displayPath), Buffer.from('display'));
	}
	const file = await prisma.fileStorage.create({
		data: {
			md5Hash: `media-${suffix}`,
			filePath,
			fileSize: 10,
	...
	const media = await prisma.media.create({
		data: { postId: post.id, fileStorageId: file.id, fileType: type, sortOrder: 0 }
	});
	let cookie = '';
	await setPostPasswordAccess(
		{
			request: new Request('http://localhost/'),
			cookies: {
				set: (_name: string, value: string) => {
					cookie = `${POST_PASSWORD_ACCESS_COOKIE}=${value}`;
				}
			}
		} as never,
		post.id
	...
	const context = (request: Request) =>
		({ params: { mediaId: media.id }, request, cookies: { get: () => undefined } }) as never;
...
test('密码帖视频受 cookie 保护，并支持 Range、416 与 HEAD', async () => {
	...
	assert.equal(
		(await stream(context(new Request('http://localhost/media/x/stream')))).status,
		404
	);
	const partial = await stream(
		context(
			new Request('http://localhost/media/x/stream', {
