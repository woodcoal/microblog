...
function recommendationCandidate(
	index: number,
	options: { author?: string; categoryId?: string | null; tagIds?: string[] } = {}
): TrendingFeedItem {
	...
	return {
		...
		customCategory: null,
		user: {
			id: options.author ?? `author_${index}`,
...
test('cold start keeps 14/6 source quotas and interleaves them deterministically', () => {
	...
	assert.deepEqual(
		first.reduce((counts, item) => ({ ...counts, [item.source]: counts[item.source] + 1 }), {
			interest: 0,
			trending: 0,
			exploration: 0
		}),
		{ interest: 0, trending: 14, exploration: 6 }
	);
...
test('blended recommendations interleave quotas and enforce author, category, and every-tag streak limits', () => {
	const candidates = Array.from({ length: 30 }, (_, index) =>
		recommendationCandidate(index, {
			author: `author_${index % 10}`,
			categoryId: `category_${index % 3}`,
			tagIds: [`tag_${index % 4}`, `secondary_${index % 5}`]
		})
	);
	for (const index of [0, 3, 6, 9, 12, 15, 18, 21, 24, 27])
		candidates[index] = recommendationCandidate(index, {
			author: `author_${index % 10}`,
			categoryId: `category_${index % 3}`,
			tagIds: ['interest', `secondary_${index % 5}`]
		});
	const items = allocateBlendedRecommendations(
		candidates,
		{ interestTagIds: ['interest'], interestCategoryIds: [] },
		10
	);
	assert.deepEqual(
		items.reduce((counts, item) => ({ ...counts, [item.source]: counts[item.source] + 1 }), {
			interest: 0,
			trending: 0,
			exploration: 0
		}),
	...
	for (const count of Object.values(
		items.reduce<Record<string, number>>(
			(counts, item) => ({
				...counts,
				[item.post.userId]: (counts[item.post.userId] ?? 0) + 1
			}),
			{}
		)
	))
	...
	for (const tagId of new Set(items.flatMap((item) => item.post.tags.map(({ tag }) => tag.id)))) {
		assertNoThreeConsecutiveValues(
			items.map((item) => (item.post.tags.some(({ tag }) => tag.id === tagId) ? tagId : null))
		);
	}
});
