...
async function user(username: string) {
	return prisma.user.create({
		data: { username, displayName: username, email: `${username}@test`, passwordHash: 'hash' }
	});
}
...
test('200-window paging excludes self interaction and pins only when explicitly requested', async () => {
	...
	await prisma.like.createMany({
		data: [
			{ userId: visitor.id, postId: 'p1' },
			{ userId: author.id, postId: 'p0' }
		]
	});
	...
	assert.equal(
		feed.items.some((post) => post.id === 'p200'),
		false
	);
	assert.deepEqual(
		(await getTrendingFeed({ viewerId: visitor.id, page: 3, pageSize: 100 })).items,
		[]
	);
	...
	assert.equal(withoutPin.total, 200);
	assert.equal(
		withoutPin.items.some((post) => post.id === 'p200'),
		true
	);
	assert.equal(
		(
...
test('skip persists only completion state and five positive signals switch to blended', async () => {
	...
	const targets = await Promise.all(
		Array.from({ length: 5 }, (_, index) => user(`target${index}`))
	);
	...
	assert.equal(profile.interestTagIds.length, 0);
	assert.equal(profile.strategy, 'cold_start');
	await prisma.follow.createMany({
		data: targets.map((target) => ({ followerId: viewer.id, followingId: target.id }))
	});
	profile = await getRecommendationProfile(viewer.id);
	assert.equal(profile.positiveSignalCount, 5);
	assert.equal(profile.strategy, 'blended');
	assert.deepEqual(profile.weights, { interest: 0.5, trending: 0.4, exploration: 0.1 });
});

test('blended output deterministically reserves 50/40/10 interest, trending, and exploration slots', async () => {
	const viewer = await user('blend_viewer');
	const author = await user('blend_author');
	const targets = await Promise.all(
		Array.from({ length: 5 }, (_, index) => user(`blend_target${index}`))
	);
	...
	await prisma.postTag.createMany({
		data: Array.from({ length: 5 }, (_, index) => ({ postId: `blend_${index}`, tagId: tag.id }))
	});
	await prisma.follow.createMany({
		data: targets.map((target) => ({ followerId: viewer.id, followingId: target.id }))
	});
	...
	assert.deepEqual(
		first.items.reduce(
			...
			{ interest: 0, trending: 0, exploration: 0 }
		),
		{ interest: 5, trending: 4, exploration: 1 }
	);
	assert.equal(
		first.items
			.find((item) => item.source === 'exploration')
