...
test('v2 config validates atomically and migrates the real three-field legacy object', () => {
	...
	assert.deepEqual(parseTrendingConfig('{"wLikes":4,"wComments":3,"decayHours":24}'), {
		version: 'v2', wLikes: 4, wBookmarks: 2, wComments: 3, decayHours: 24 * Math.LN2
	});
	...
	assert.deepEqual(parseTrendingConfig('{"version":"v2","wLikes":4,"wBookmarks":2,"wComments":3,"decayHours":720}'), { version: 'v2', wLikes: 4, wBookmarks: 2, wComments: 3, decayHours: 720 });
	assert.deepEqual(parseTrendingConfig('{"version":"v2","wLikes":4,"wBookmarks":2,"wComments":3,"decayHours":721}'), DEFAULT_TRENDING_CONFIG);
	assert.deepEqual(parseTrendingConfig('{"version":"v2","wLikes":0,"wBookmarks":0,"wComments":0,"decayHours":48}'), DEFAULT_TRENDING_CONFIG);
	assert.deepEqual(parseTrendingConfig('{"wLikes":0,"wComments":0,"decayHours":24}'), DEFAULT_TRENDING_CONFIG);
});
...
test('migrated legacy decay curve, v2 half-life, and ties have deterministic order', () => {
	...
	const legacyFresh = calculateTrendingScore({ likes: 4, bookmarks: 0, comments: 1 }, new Date(now), legacy, now);
	const elapsedHours = 17;
	const migratedScore = calculateTrendingScore({ likes: 4, bookmarks: 0, comments: 1 }, new Date(now - elapsedHours * 3_600_000), legacy, now);
